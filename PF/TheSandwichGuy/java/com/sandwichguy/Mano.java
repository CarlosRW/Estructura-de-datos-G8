...
public class Mano {
    ...
    public void ordenar() {
        Collections.sort(cartas, (c1, c2) -> Integer.compare(c1.getValorParaSandwich(), c2.getValorParaSandwich()));
    }
    ...
    public boolean existeSandwichValido() {
        ...
        // Verificar todas las combinaciones de 3 cartas
        List<Carta> todas = new java.util.ArrayList<>(cartas);
        int n = todas.size();
        for (int i = 0; i < n; i++) {
            for (int j = i + 1; j < n; j++) {
                for (int k = j + 1; k < n; k++) {
                    List<Carta> tripleta = new java.util.ArrayList<>();
